...
class Resource(db.DynamicDocument):
    ...
    download_uuid = db.StringField(unique=True)  # uuid3 防止重复
    show_times = db.IntField(default=0)  # 显示次数
    like_times = db.IntField(default=0)  # 喜欢次数
    dislike_times = db.IntField(default=0)  # 不喜欢次数
    report_times = db.IntField(default=0)  # 举报次数
